...
typedef struct
{
	 ...
	 */
       /*statistics. These counters should be wide enough that they cant overflow during a sinle bgwriter cycle. */
...
StrategyGetBuffer(void)
{
	...
	for(;;)
	{
		...
		//buf->FREEPREV = FREEPREV_NOT_IN_LIST;
		/*
		 ...
		 */
		LockBufHdr(buf);
		if (buf->refcount == 0) //--U70331249-- WE dont need usage_count here because we are eliminating it from our new LRU algorithm
	{
		elog(LOG, "Get buf %d\n", buf->buf_id); //--U70331249-- This line writes to log whenever a new buffer appears.
			return buf;
	}
		UnlockBufHdr(buf);
	}
...
StrategyFreeBuffer(volatile BufferDesc *buf, bool at_head)
{
	...
	{
	  /*--U70331249-- This elog code is added to check the implementation of buffer manager*/
		elog(LOG, "Add buf %d\n",buf->buf_id);
		...
		else  //--U70331249 -- as at_head is always false this will only execute
		{
