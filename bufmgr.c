...
retry:
	...
	buf->flags = 0;
	//buf->usage_count = 0;//U70331249 we are not using usage_count
	...
	if (BufferIsValid(buffer))
	{
		if (BufferIsLocal(buffer))
		{
			...
			/*if (LocalRefCount[-buffer - 1] == 0 &&
				...
				bufHdr->usage_count++;*///U70331249--we dont use these lines as we not using usage_count++ in our implementation
		}
		else
		{
			Assert(PrivateRefCount[buffer - 1] > 0);
...
UnpinBuffer(volatile BufferDesc *buf, bool fixOwner, bool normalAccess)
{
	...
	if (PrivateRefCount[b] == 0)
	{
		...
		if (normalAccess)
		{
			...
			{*/ //U70331249-- We can elimnate these lines as we are not using usage_count in our code//
				/* VACUUM accesses don't bump usage count, instead... */
				if (buf->refcount == 0 ) //&& buf->usage_count == 0)
		//U70331249 eliminating usage_count
...
SyncOneBuffer(int buf_id, bool skip_pinned)
{
	...
	if (skip_pinned &&
		(bufHdr->refcount != 0)) //--U70331249--usage_count eliminated
	{
		UnlockBufHdr(bufHdr);
		return false;
	}
...
ReleaseBuffer(Buffer buffer)
{
	...
	if (BufferIsLocal(buffer))
	{
		...
		/*if (LocalRefCount[-buffer - 1] == 0 &&
			bufHdr->usage_count < BM_MAX_USAGE_COUNT)
			bufHdr->usage_count++;*///U70331249 We are not using usage_count in our new implementation. So we don't need these steps
		return;
	}
